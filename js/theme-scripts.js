...
tjq(window).load(function() {
    ...
    function displayTestimonials() {
        ...
        try {
            if (tjq('.testimonial.style4').length > 0 && tjq('.testimonial.style4').is(":visible")) {
                tjq('.testimonial.style4').flexslider({
                    namespace: "testimonial-",
                    controlNav: false,
                    animationLoop: false,
                    directionNav: true,
    ...
    tjq('.modal').on('shown.bs.modal', function () {
        displayPhotoGallery(tjq(this).find(".photo-gallery"));
    });

    tjq('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        var contentId = tjq(e.target).attr("href");
        if(contentId == "#tour-itinerary"){
            var contentId = tjq(".tour-itinerary-content .tabs li.active").find("a").attr("href");
        }
